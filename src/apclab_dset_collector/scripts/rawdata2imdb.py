...
class RawData2IMDB():
    ...
    def get_imdb(self):
        ...
        with open(raw_data_file) as raw_data, open(output_file_path, "a") as output_file:
            ...
            for row in reader[0:377]: #from 10:11
                right_image_name = row['right_image_name']
                left_image_name = row['left_image_name']
                ...
                lyap = np.amin(lyap_buff) #returns the minimum of the lyap_buff array
                self.lyap_func.X_T = X_Ts[np.argmin(lyap_buff)]
                # dataset augmentation: if True, the generated imdb will be 6x bigger
                output_hearder = ['right_image_name', 'left_image_name', 'lyap', 'pd_1', 'pd_2','pd_3',
                ...
                if self.dataset_focus == 'global':
                    ...
                    new_right_image_name  = right_image_name
		    new_left_image_name = left_image_name
                ...
                if meet_collect_criterion:
                    ...
                    new_row = [right_image_name]+[left_image_name]+[lyap]+pd_Lyap+[row['shoulder_pan_joint'],
                    	                   row['shoulder_lift_joint'],
                                           row['elbow_joint'],
                                           row['wrist_1_joint'],
                                           row['wrist_2_joint'],
    ...
    def cmpt_mean_value_point(self, joint_config, joint_index, X_T, delta):
        ...
        theta = joint_config[joint_index,0]
        cos = math.cos(theta)
        sin = math.sin(theta)
        # Rotation matrix Z Linearisation
        C = np.eye(4); C[0:2, 0:2] = np.array([[cos, sin],[-sin, cos]])
        D = np.zeros([4, 4]); D[0:2, 0:2] = np.array([[-sin, cos],[-cos, -sin]])
        E = np.zeros([4, 4]); E[0:2, 0:2] = np.array([[sin, -cos],[cos, sin]])
        F = np.zeros([4, 4]); F[0:2, 0:2] = np.array([[cos, sin],[-sin, cos]])
        # for a quadratic equation ax^2+x^2+c
        M1 = np.transpose(inv_B.dot(C).dot(inv_A).dot(G))
        M2 = np.transpose(inv_B.dot(D).dot(inv_A).dot(G))
        M3 = inv_B.dot(E).dot(inv_A).dot(G)
        M4 = inv_B.dot(F).dot(inv_A).dot(G)
        a = np.trace(M2.dot(M4))
        b = np.trace(M2.dot(M3) + (M1-np.eye(4)).dot(M4))
        c = np.trace((M1-np.eye(4)).dot(M3))
        # RHS
        ...
        # coefficients p[] for quadratic equation
        c = c + slope/2.0
        mean_value_point = -c/b+theta
        phi = -c/b
        pd = -2.0*np.trace((M1+phi*M2-np.eye(4)).dot(M3+phi*M4))
        print('###########')
        print('recovered pd: %.4f' % pd)
        # print('approx %.4f' % mean_value_point)
        # p = [a, b, c]
        # roots = np.roots(p)
        # mean_value_point = None
        # real_roots = roots[np.isreal(roots)].real
        # print(real_roots)
        # mean_value_points = real_roots+theta
        # left_end = joint_config[joint_index, 0] - 0.5
        # right_end = joint_config[joint_index, 0] + 0.5
        ...
        c[joint_index, 0] = mean_value_point
...
def main(args):
    folder_name = 'dset' #args[1]
    dataset_focus = 'global' #args[2]
